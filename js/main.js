...
function addUpCreditsGpa() {
    ...
    //counting credits unit row
    const creditUnit = document.querySelectorAll('[name = creditUnit]');

    //credits row
    const grade = document.querySelectorAll('[name = grade]');

    let totalUnit = document.querySelectorAll("[name = totalUnit]").value = "";
    let gpa = document.querySelectorAll("[name = gpa]").value = "";

    let option1 = "";
    let option2 = "";

    let a = []; //intiallizing an empty array //below looping thruogh the sele
...
function gradeToPoints(grade) {
    if ("A" == grade) {
        ...
        //XXX! Should we throw an expection here
        return Invalid;
    }
}
